...
class SlidingWindowCounter {
  ...
  getWindowInfo() {
    ...
    return {
      ...
      nextAvailableTime:
        activeRequests.length >= this.limit ? activeRequests[0] + this.windowSizeMs - now : 0,
    };
  }
}
...
class AdvancedRateLimiter {
  ...
  setModuleLimit(moduleName, logsPerMinute) {
    ...
    this.moduleCounters.set(moduleName, new SlidingWindowCounter(this.windowSizeMs, logsPerMinute));
  }
  ...
  getStats() {
    ...
    const allowRate =
      totalRequests > 0 ? ((this.stats.allowed / totalRequests) * 100).toFixed(1) : 0;
