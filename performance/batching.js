...
export class BatchingManager extends EventEmitter {
  ...
  async add(item, options = {}) {
    ...
    // Check queue size
    const totalSize =
      Array.from(this.queues.values()).reduce((sum, q) => sum + q.length, 0) +
  ...
  getStatistics() {
    const avgBatchSize =
      this.stats.totalBatches > 0 ? this.stats.itemsProcessed / this.stats.totalBatches : 0;

    return {
      ...this.stats,
      averageBatchSize: avgBatchSize.toFixed(2),
      currentQueueSize: Array.from(this.queues.values()).reduce((sum, q) => sum + q.length, 0),
  ...
  async _startProcessing() {
    ...
    try {
      ...
      const totalItems = Array.from(this.queues.values()).reduce((sum, q) => sum + q.length, 0);
