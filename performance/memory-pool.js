...
export class MemoryPoolManager extends EventEmitter {
  ...
  _createObject(type) {
    const factory =
      typeof this.objectFactory === 'function'
        ? this.objectFactory
  ...
  release(obj) {
    ...
    // Reset object state
    const resetFn =
      typeof this.resetFunction === 'function'
        ? this.resetFunction
  ...
  _performGC() {
    ...
    if (this.leakDetectionEnabled) {
      ...
      for (const [id, tracked] of this.checkedOutObjects) {
        if (now - tracked.acquiredAt > 60000) {
          // 1 minute timeout
  ...
  getStatistics() {
    ...
    const hitRate =
      this.stats.poolHits + this.stats.poolMisses > 0
  ...
  validate() {
    ...
    if (this.leakDetectionEnabled) {
      const now = Date.now();
      for (const [id, tracked] of this.checkedOutObjects) {
        if (now - tracked.acquiredAt > 300000) {
          // 5 minutes
