...
export class LazyLoadingManager extends EventEmitter {
  ...
  async load(name) {
    ...
    try {
      const instance = await Promise.race([loadPromise, this._createTimeoutPromise(name)]);
      return instance;
    } finally {
  ...
  getStatistics() {
    const avgTime =
      this.stats.loadedModules > 0 ? this.stats.totalLoadTime / this.stats.loadedModules : 0;
