...
export class AdaptiveSamplingManager extends EventEmitter {
    ...
    _updateRuleStats(ruleName, sampled) {
        ...
        stats.effectiveness = Math.abs(stats.sampledCount - expectedSampled) / Math.max(expectedSampled, 1);
    }
    ...
    getAdaptiveRecommendation(systemMetrics = {}) {
        ...
        if (memoryUsage > this.memoryThreshold) {
            ...
            recommendation.adjustmentFactor = Math.max(0.05, recommendation.adjustmentFactor * (1.0 - (overage / 100) * 0.4));
            recommendation.shouldAdjust = true;
        ...
        if (queueDepth > this.queueDepthThreshold) {
            ...
            recommendation.adjustmentFactor = Math.max(0.05, recommendation.adjustmentFactor * (1.0 / depthRatio));
            recommendation.shouldAdjust = true;
    ...
    getHistory(filter = {}) {
        return this.history.filter(entry => {
            ...
            if (filter.ruleName && entry.details && entry.details.ruleName !== filter.ruleName) return false;
            return true;
        });
    }
