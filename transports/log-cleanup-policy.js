...
class LogCleanupPolicy {
  ...
  _validateOptions(options) {
    if (options.maxAge && (typeof options.maxAge !== 'number' || options.maxAge <= 0)) {
      throw new LoggingError('INVALID_CONFIG', 'maxAge must be a positive number', {
    ...
    if (
      ...
      (typeof options.maxTotalSize !== 'number' || options.maxTotalSize <= 0)
    ) {
      throw new LoggingError('INVALID_CONFIG', 'maxTotalSize must be a positive number', {
    ...
    if (
      ...
      (typeof options.checkInterval !== 'number' || options.checkInterval <= 0)
    ) {
      throw new LoggingError('INVALID_CONFIG', 'checkInterval must be a positive number', {
  ...
  async start(directory, onCleanup = null) {
    if (this._isRunning) {
      throw new LoggingError('ALREADY_RUNNING', 'Cleanup scheduler is already running');
    }
  ...
  async getDirectoryStats(directory) {
    try {
      ...
      throw new LoggingError('STATS_ERROR', `Failed to get directory stats: ${error.message}`, {
        originalError: error,
        directory,
      });
    }
