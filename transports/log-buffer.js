...
class LogBuffer {
  ...
  _validateOptions(options) {
    if (options.maxSize && (typeof options.maxSize !== 'number' || options.maxSize < 1)) {
      throw new LoggingError('INVALID_CONFIG', 'maxSize must be a number >= 1', {
    ...
    if (
      ...
      (typeof options.flushInterval !== 'number' || options.flushInterval < 1)
    ) {
      throw new LoggingError('INVALID_CONFIG', 'flushInterval must be a number >= 1', {
        receivedFlushInterval: options.flushInterval,
      });
    }

    if (options.onFlush && typeof options.onFlush !== 'function') {
      throw new LoggingError('INVALID_CONFIG', 'onFlush must be a function');
    }
  }
  ...
  add(entry) {
    if (!entry) {
      throw new LoggingError('INVALID_ENTRY', 'Entry cannot be null or undefined');
    }

    if (this._isPaused) {
      throw new LoggingError('BUFFER_PAUSED', 'Buffer is paused and cannot accept new entries');
    }
  ...
  addBatch(entries) {
    if (!Array.isArray(entries)) {
      throw new LoggingError('INVALID_INPUT', 'entries must be an array');
    }

    if (entries.length === 0) {
      throw new LoggingError('EMPTY_BATCH', 'Batch cannot be empty');
    }

    for (const entry of entries) {
      if (!entry) {
        throw new LoggingError('INVALID_ENTRY', 'Batch contains null or undefined entry');
      }
    }
  ...
  start() {
    if (this._isRunning) {
      throw new LoggingError('ALREADY_STARTED', 'Buffer timer is already running');
    }
  ...
  getStats() {
    return {
      ...
      averageFlushSize:
        this._stats.flushCount > 0 ? this._stats.entriesFlushed / this._stats.flushCount : 0,
      efficiency:
        this._stats.entriesAdded > 0
