...
class FileTransportImpl extends BaseTransport {
    ...
    async log(entry) {
        try {
            ...
            if (typeof process === 'undefined' || !process.versions || !process.versions.node) {
                ...
                this.handleError(new LoggingError('FileTransport is not supported in non-Node environments'), entry);
                return;
            }
    ...
    async getLogFilePath(moduleName) {
        try {
            ...
            throw new LoggingError(`Failed to create log file path for module "${moduleName}"`, error instanceof Error ? error : new Error(String(error)));
        }
    }
