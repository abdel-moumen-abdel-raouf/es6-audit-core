...
class LogArchiver {
    ...
    try {
      ...
      if (results.stats.originalSize > 0) {
        results.stats.compressionRatio =
          (1 - results.stats.compressedSize / results.stats.originalSize) * 100;
      }
  ...
  static async compressFile(filePath, options = {}) {
    try {
      ...
      if (fs.existsSync(archivePath)) {
        throw new LoggingError('ARCHIVE_EXISTS', `Archive already exists: ${archivePath}`);
      }
      ...
      // Verify compression
      if (!fs.existsSync(archivePath)) {
        throw new LoggingError('COMPRESSION_FAILED', 'Archive file was not created');
      }
  ...
  static async extractFile(archivePath, extractPath = null) {
    try {
      ...
      if (!archivePath.endsWith('.gz')) {
        throw new LoggingError('INVALID_FORMAT', 'File must be a .gz archive');
      }

      extractPath = extractPath || archivePath.slice(0, -3);

      if (fs.existsSync(extractPath)) {
        throw new LoggingError('FILE_EXISTS', `Extract destination already exists: ${extractPath}`);
      }

      await LogArchiver._gunzipFile(archivePath, extractPath);

      if (!fs.existsSync(extractPath)) {
        throw new LoggingError('EXTRACTION_FAILED', 'Extract file was not created');
      }
  ...
  static getArchiveStats(directory, options = {}) {
    try {
      ...
      throw new LoggingError('STATS_ERROR', `Failed to get archive stats: ${error.message}`, {
        originalError: error,
        directory,
      });
    }
  ...
  static async _gzipFile(filePath, archivePath) {
    return new Promise((resolve, reject) => {
      ...
      source.pipe(gzip).pipe(destination).on('error', reject).on('finish', resolve);
  ...
  static async _gunzipFile(archivePath, filePath) {
    return new Promise((resolve, reject) => {
      ...
      source.pipe(gunzip).pipe(destination).on('error', reject).on('finish', resolve);
  ...
  static _validateDirectory(directory) {
    if (!directory || typeof directory !== 'string') {
      throw new LoggingError('INVALID_DIRECTORY', 'Directory must be a non-empty string', {
        receivedDirectory: directory,
      });
    }

    if (!fs.existsSync(directory)) {
      throw new LoggingError('DIRECTORY_NOT_FOUND', `Directory not found: ${directory}`);
    }
  }
  ...
  static _validateFile(filePath) {
    if (!filePath || typeof filePath !== 'string') {
      throw new LoggingError('INVALID_FILE', 'File path must be a non-empty string', {
        receivedFilePath: filePath,
      });
    }

    if (!fs.existsSync(filePath)) {
      throw new LoggingError('FILE_NOT_FOUND', `File not found: ${filePath}`);
    }
  }
