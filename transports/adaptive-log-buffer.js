...
export class AdaptiveLogBuffer {
   ...
   */
  isFull() {
    return this.buffer.length >= this.maxSize || this.memoryUsage >= this.maxMemory;
  }
  ...
  printStatistics() {
    ...
    console.log(
      `Memory: ${stats.memoryUsageMB}MB/${stats.maxMemoryMB}MB (${stats.utilizationByMemory})`
    );
    console.log(
      `Status: ${stats.isPaused ? 'PAUSED (backpressure)' : 'ACTIVE'} | Full: ${stats.isFull}`
    );
    console.log(`\nStatistics:`);
